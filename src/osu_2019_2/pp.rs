...
impl<'m> FxPP<'m> {
    ...
    pub fn calculate(mut self) -> OsuPerformanceAttributes {
        ...
        pp *= match self.map.title.to_lowercase().as_str() {
            ...
            title if title.contains("Yomi Yori") => 0.93, //* 17904 -> 16651 and nerf patterns
        ...
        if self.attributes.is_none() || !self.mods.rx() {
            let attributes = stars(self.map, self.mods, self.passed_objects);
            self.attributes.replace(attributes);
        }
    ...
    fn compute_aim_value(&self, total_hits: f32, effective_miss_count: f32) -> f32 {
        ...
        aim_value *= 0.925 + attributes.od as f32 * attributes.od as f32 / 2470.0;
